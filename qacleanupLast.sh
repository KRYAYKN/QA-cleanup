...
# Step 3: Find and Delete Merge Commits for Failed Branches
echo "Finding and deleting merge commits for failed branches..."
    ...
    if [[ -n "$MERGE_COMMITS" ]]; then
        ...
        for commit in $MERGE_COMMITS; do
            ...
            git reset --hard $(git rev-parse "$commit^") || {
                echo "Failed to reset commit $commit. Resolve manually."
                exit 1
            }
